        ...
        <div className="overflow-hidden relative">
          <div className="flex animate-marquee whitespace-nowrap">
            {[...Array(3)].map((_, repeatIdx) => (
              <div key={repeatIdx} className="flex whitespace-nowrap">
                {items.map((item) => {
                  const isFile = !!item.link.match(/\.(pdf|jpg|jpeg|png)$/i);
                  return (
                    <a
                      key={item._id + repeatIdx}
                      href={item.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      download={isFile ? true : undefined} // enable download for files
